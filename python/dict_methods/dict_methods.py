...
def send_to_store(cart: dict, aisle_mapping: dict) -> dict:
    ...
    for product in cart:
        ...
        for mapping in aisle_mapping:
            if mapping == product:
                cart[product].extend(aisle_mapping[mapping])
    return dict(sorted(cart.items(), reverse = 1))
...
def update_store_inventory(fulfillment_cart: dict, store_inventory: dict) -> dict:
    ...
    for product in fulfillment_cart:
        for inventory in store_inventory:
            if inventory == product and store_inventory[inventory][0] != "Out of Stock":
                store_inventory[inventory][0] -= fulfillment_cart[product][0]
                if store_inventory[inventory][0] == 0:
                    store_inventory[inventory][0] = "Out of Stock"
